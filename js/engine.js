...
(() => {
  ...
  function objectiveProgress(def, st, index) {
    ...
    if (obj.kind === "fetch") {
      ...
      if ((st.status === "completed" || st.status === "failed") && slot.current) return slot.current;
  ...
  const Quests = {
    ...
    start(id) {
      ...
      if (st && st.status === "failed" && !def.allowRestartOnFail) return false;
    ...
    abandon(id) {
      const def = questDef(id);
      if (!def || !def.canAbandon) return false;
      return this.fail(id, "Abandoned.");
    },
  ...
  async function menuJournal() {
    ...
    function showJournalDetail(title, html, canAbandon) {
      return new Promise((resolve) => {
        const win = el("div", "win statuswin");
        win.style.left = "50%";
        win.style.top = "50%";
        win.style.transform = "translate(-50%, -50%)";
        win.appendChild(el("div", "win-title", esc(title)));
        win.appendChild(el("div", "journal-detail", html));
        const btnRow = el("div", "modal-btns");
        const backBtn = el("button", "primary", "Back");
        const buttons = [backBtn];
        let selIdx = 0;
        function finish(result) { removeUI(ui); resolve(result); }
        function refreshButtons() {
          buttons.forEach((btn, i) => {
            btn.style.outline = i === selIdx ? "2px solid #9ab8f0" : "none";
            btn.style.background = i === selIdx ? "rgba(90,130,220,0.45)" : "";
          });
          if (buttons[selIdx] && buttons[selIdx].focus) buttons[selIdx].focus();
        }
        function moveSel(d) {
          selIdx = (selIdx + d + buttons.length) % buttons.length;
          sysSe("cursor");
          refreshButtons();
        }
        ...
        if (canAbandon) {
          ...
          abandonBtn.addEventListener("click", () => finish("abandon"));
        ...
        const ui = {
          el: win,
          onKey(k) {
            if (k === "cancel") finish("back");
            else if (k === "left" || k === "up") moveSel(-1);
            else if (k === "right" || k === "down") moveSel(1);
            else if (k === "ok") buttons[selIdx].click();
          },
        };
        uiLayer.appendChild(win);
        pushUI(ui);
    ...
    while (true) {
      const gi = await showList(groups.map((g) => ({ label: g.title })), { title: "Journal", className: "itemwin" });
      if (gi < 0) return;
      const group = groups[gi];
      const quests = (proj.quests || []).filter((q) => {
        const st = questState(q.id);
        return st && st.status === group.key;
      });
      if (!quests.length) {
        await showMessage("", "No " + group.title.toLowerCase() + ".");
        continue;
      }
      while (true) {
        const qi = await showList(quests.map((q) => ({
          html: "<b>" + esc(q.name || "Quest") + "</b>" +
            (q.shortDesc ? '<br><span class="dim">' + esc(q.shortDesc) + "</span>" : "") +
            Quests.objectiveDisplay(q.id).map((obj) => '<br><span' + (obj.done ? ' class="dim"' : '') + '>' +
              (obj.done ? '&#10003; ' : '&#8226; ') + esc(obj.text) + ' <span class="cnt">' + obj.current + ' / ' + obj.total + '</span></span>').join(""),
          help: q.desc || q.shortDesc || "",
        })), { title: group.title, className: "itemwin" });
        if (qi < 0) break;
        const q = quests[qi];
        const failSummary = group.key === "failed" ? Quests.failSummary(q.id) : "";
        const detailHtml = "<b>" + esc(q.name || "Quest") + "</b>" +
          (q.shortDesc ? "<br>" + esc(q.shortDesc) : "") +
          (q.desc ? "<br><br>" + esc(q.desc).replace(/\n/g, "<br>") : "") +
          Quests.objectiveDisplay(q.id).map((obj) => '<br><span' + (obj.done ? ' class="dim"' : '') + '>' +
            (obj.done ? '&#10003; ' : '&#8226; ') + esc(obj.text) + ' <span class="cnt">' + obj.current + ' / ' + obj.total + '</span></span>').join("") +
          (failSummary ? "<br><br><span class=\"dim\">Outcome: " + esc(failSummary) + "</span>" : "");
        if (group.key === "active" && q.canAbandon) {
          const act = await showJournalDetail(q.name || "Quest", detailHtml, true);
          if (act === "abandon") {
            if (Quests.abandon(q.id)) await showMessage("", "Quest abandoned.");
            return "close";
          }
        } else {
          await showJournalDetail(q.name || "Quest", detailHtml, false);
        }
      }
    }
  }
